					...
					<table id="courseEval" class="dattable table table-striped thead-dark  w-100">
						...
						@foreach($rows as $index => $row)
							...
							<td>{{ $index +1 }}</td>
                                      ...
                                      >edit</a>
                                        <a href="#" onclick="destroy('this City','{{$row->id}}')" class="btn d-inline-block btn-danger">delete</a>
                                         <form id="delete_{{$row->id}}" action="{{ route('city.destroy', $row->id) }}"  method="POST" style="display: none;">
									@csrf
									@method('DELETE')
