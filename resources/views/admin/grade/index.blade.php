				...
				<div class="table-responsive">
					<table id="courseEval" class="dattable table table-striped thead-dark  w-100">
						...
						@foreach($rows as $index => $row)
							<tr>
							<td>{{ $index +1 }}</td>
			  						<td>{{$row->grade_text}}</td>
                 
									  <td>
                                      <a  href="#"  data-toggle="modal" data-target="#addModel{{$row->id}}" class="btn btn-info d-inline-block" 
                                      >edit</a>
                                        <a href="#" onclick="destroy('this Grade','{{$row->id}}')" class="btn d-inline-block btn-danger">delete</a>
                                         <form id="delete_{{$row->id}}" action="{{ route('grade.destroy', $row->id) }}"  method="POST" style="display: none;">
									@csrf
									@method('DELETE')
  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <div class="ms-auth-container row no-gutters">
          ...
          <form action="{{route('grade.update',$row->id)}}" method="POST">
                @method('PUT')
				  @csrf
                      ...
                      <div class="col-md-12">
                          <div class="form-group">
                              ...
                              <div class="input-group">
                                  <input type="text" id="newTitle" name="grade_text" value="{{$row->grade_text}}" class="form-control"
                                     placeholder="Grade Text">
                              </div>
                          </div>
                      </div>
                      ...
                      <div class="ms-auth-container row">
                          ...
                          <div class="col-md-12">
                              <div class="form-group">
                                  <label  >{{ __('Name') }}</label>
                                  <div class="input-group">
                                      <input type="text" id="newTitle" name="grade_text" class="form-control"
                                         placeholder="Grade Text">
                                  </div>
                              </div>
                          </div>
